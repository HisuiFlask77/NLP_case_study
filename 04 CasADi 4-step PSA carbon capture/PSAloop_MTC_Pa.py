...
class VPSA_Robust(VPSA_Simulator):
    ...
    def simulate_css(self, cycles=1000, make_plot=False):
        y_init_val = 0.0
        P_init_val = 1e4
        T_init_val = 298.15
...
def run_single_simulation(n_val):
    """
    Stand-alone function for parallel execution.
    Must be defined outside the if-name-main block but after the classes.
    """
    start_time = time.time()
    # Create simulator instance inside the child process
...
if __name__ == "__main__":
    ...
    print(f"Starting parallel execution for N = {N_values}...")
    print(f"{'N':<5} | {'Cycles':<10} | {'Time (s)':<10} | {'Final Error':<12}")
    print("-" * 55)

    wall_start = time.time()

    # Use ProcessPoolExecutor to distribute tasks
    with ProcessPoolExecutor() as executor:
        # map runs the function for each value in the list
        parallel_results = list(executor.map(run_single_simulation, N_values))
    ...
    for res in parallel_results:
        n_val = res['n']
        results[n_val] = res
        print(f"{n_val:<5} | {res['cycles']:<10} | {res['time']:<10.2f} | {res['final_error']:<12.2e}")

    print("-" * 55)
    print(f"Total Parallel Wall Time: {wall_elapsed:.2f} seconds")

    # --- Plotting ---
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))

    times = [results[n]['time'] for n in N_values]
    ax1.plot(N_values, times, 'o-', color='crimson', label='Serial Time per Case')
    ax1.axhline(y=wall_elapsed, color='blue', linestyle='--', label='Parallel Wall Time')
    ax1.set_xlabel('N (Number of Nodes)')
    ax1.set_ylabel('Execution Time (s)')
    ax1.set_title('Computational Efficiency Analysis')
    ax1.legend()
    ax1.grid(True)
    ...
    ax2.set_title('Mesh Convergence: Final CO2 Profile')
    ax2.legend()
    ax2.grid(True)
